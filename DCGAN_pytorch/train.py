...
def train(args):
    ...
    if mode == 'train':
        # generator output인 tanh(-1~1) 값과 D input scale을 맞춰주기 위해 normalize
    ...
    if mode == 'train':  # else : 'test'
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for batch, data in enumerate(loader_train, 1):  # loader_train에는 real-image data가 들어있다.
                ...
                loss_G = fn_loss(pred_fake,
                                 torch.ones_like(pred_fake))  # generator는 생성한 가짜 이미지가 진짜(True,1)로 분류되게 학습해야한다.
                ...
                print(
                    "TRAIN: EPOCH %04d / %04d | BATCH %04d / %04d | G_LOSS %.4f | D_fake_LOSS %.4f | D_real_LOSS %.4f " %
                    (epoch, num_epoch, batch, num_batch_train, np.mean(loss_G_train), np.mean(loss_D_fake_train),
                     np.mean(loss_D_real_train)))

                if batch % 10 == 0:
                    # Tensorboard 저장하기
                    output = fn_tonumpy(fn_denorm(output, mean=0.5,
                                                  std=0.5)).squeeze()  # generator로 생성된 output은 마지막 layer에 tanh를 거치며 normalize된다.
                    # 따라서 denorm을 통해 복원
